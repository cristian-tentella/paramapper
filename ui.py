    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if item.column != 'NONE':
            ...
            if col_meta and col_meta.data_type == 'NUMERIC':
                ...
                info_row.active = False
                info_row.alignment = 'RIGHT'
                info_row.label(text=f"Range: {col_meta.min_val:.2f} ↔ {col_meta.max_val:.2f}", icon='INFO')
...
class PARAMAPPER_OT_add_filter(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        context.active_object.paramapper.active_filter_index = len(context.active_object.paramapper.filters) - 1
        return {'FINISHED'}
...
class PARAMAPPER_PT_filters(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.template_list("PARAMAPPER_UL_filter_list", "", props, "filters", props, "active_filter_index", rows=3)
