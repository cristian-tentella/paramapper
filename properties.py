...
def update_fast(self, context):
    if not getattr(self, "auto_update", False) or not getattr(self, "dataset_has_been_parsed", False):
        return
    ...
    if mat_text and mat_text.use_nodes:
        node_principled = next((n for n in mat_text.node_tree.nodes if n.type == 'BSDF_PRINCIPLED'), None)
        if node_principled:
    ...
    if mat_bbox and mat_bbox.use_nodes:
        node_principled = next((n for n in mat_bbox.node_tree.nodes if n.type == 'BSDF_PRINCIPLED'), None)
        if node_principled:
...
class PARAMAPPER_PG_Settings(bpy.types.PropertyGroup):
    ...
    map_x: bpy.props.EnumProperty(name="X Axis", items=_get_numeric_columns, update=update_infographic) # type: ignore
    map_y: bpy.props.EnumProperty(name="Y Axis", items=_get_numeric_columns, update=update_infographic) # type: ignore
    map_z: bpy.props.EnumProperty(name="Z Axis", items=_get_numeric_columns, update=update_infographic) # type: ignore
    ...
    map_text: bpy.props.EnumProperty(
        name="Text Label",
        items=_get_categorical_columns,
        update=update_infographic
    ) # type: ignore
